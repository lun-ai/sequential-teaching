%% lists are easier to work with in prolog, commas serves as natural separators in a list
...
%% left_bag, right_bag (FIFO queues) serve as parser buffers into which integers are parsed from lt expressions
...
%% merge takes two expressions from expr of A and returns one expression in memory of B
%merger(A,B):-parse_exprs(A,C),merger_1(C,B).
%merger_1(A,B):-compare_nums(A,C),merger_1(C,B).
%merger_1(A,B):-drop_bag_remaining(A,C),drop_bag_remaining(C,B).

%% hand-written sorter/2 in h22 chain, size 4
%sorter(A,B):-single_expr(A,C),single_expr(C,B).
%sorter(A,B):-sorter_1(A,C),sorter(C,B).
%sorter_1(A,B):-merger(A,C),sorter_1(C,B).
%sorter_1(A,B):-merger(A,C),recycle_memory(C,B).

%% sorter learned by metagol given merger as background, size 3
%sorter(A,B):-merger(A,C),sorter(C,B).
%sorter(A,B):-recycle_memory(A,C),sorter(C,B).
%sorter(A,B):-single_expr(A,C),single_expr(C,B).

%% sorter learned by metagol without merger in background, size 5
%% deduction of examples has to been performed by Metagol's meta-interpreter
%% otherwise, the program below does not terminate
...
%% some quick test inputs, short & inversely ordered & long sequences
test_short_seq1:-
    A = [expr(["23"]),energy(0),left_bag([]),right_bag([]),memory([])],
    B = [expr(["23"]),energy(_),left_bag(_),right_bag(_),memory(_)],
    sorter(A,B).
test_short_seq2:-
    A = [expr(["15","13","7"]),energy(0),left_bag([]),right_bag([]),memory([])],
    B = [expr(["7<13<15"]),energy(_),left_bag(_),right_bag(_),memory(_)],
    sorter(A,B).

test_long_seq:-
    A = [expr(["23","4","93","40","1","171","34","82","5"]),energy(0),left_bag([]),right_bag([]),memory([])],
    B = [expr(["1<4<5<23<34<40<82<93<171"]),energy(_),left_bag(_),right_bag(_),memory(_)],
    sorter(A,B).
