...
:- module(metaopt,[learn_task/2,learn_seq/2,metagol/2,metaopt/2,metagolo/2,pprint/1,deduce_atom/2,get_best_program/1,learn/4,op(950,fx,'@')]).
...
deduce_atom(Atom,Prog):-
    set_best_cost(inf),
    iterator(MaxN),
    maplist(atom_to_list,[Atom],[Atom1]),
    target_predicate([Atom1],P/A),
    invented_symbols(MaxN,P/A,Sig),
    prove_deduce([Atom1],Sig,Prog,_).
...
convert_to_interpreted((Head:-true),metaopt:(interpreted_bk(HeadAsList,[]))):-!,
    ho_atom_to_list(Head,HeadAsList).
convert_to_interpreted((Head:-Body),metaopt:(interpreted_bk(HeadAsList,BodyList2))):-
    ho_atom_to_list(Head,HeadAsList),
...
assert_prims(Prog):-
    findall(P/A,(member(sub(_Name,P,A,_MetaSub,_Sig),Prog)),Prims),!,
...
retract_old:-
  retractall(user:sorter(_,_)),
  retractall(user:sorter_1(_,_)),
  retractall(user:sorter_2(_,_)),
  retractall(user:sorter_3(_,_)),
  retractall(user:sorter_4(_,_)),
  retractall(user:sorter_5(_,_)),
  retractall(user:sorter_6(_,_)),
  retractall(user:sorter_7(_,_)),
  retractall(user:sorter_8(_,_)),
  retractall(user:sorter_9(_,_)).
