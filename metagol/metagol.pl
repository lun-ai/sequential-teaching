%% This file is a copyrighted under the BSD 3-clause licence, details of which can be found under the parent directory.
...
deduce_atom(Atom,Prog):-
    setup,
    max_clauses(MaxN),
    make_atoms([Atom],[Atom1]),
    target_predicate([Atom1],P/A),
    invented_symbols(MaxN,P/A,Sig),
    assert_sig_types(Sig),
    deduce_atom(Atom1,Sig,Prog).
...
prove_aux(p(P,A,Args,Path),_FullSig,_Sig,_MaxN,N,N,Prog,Prog):-
    (nonvar(P) -> type(P,A,body_pred); true),
    body_pred_call(P,Args),
    \+memberchk([P|Args],Path).
...
assert_body_preds(S1):-
    forall(member(P/A,S1),(
        ...
        (current_predicate(P/A) -> (
            (\+(type(P,A,body_pred)) -> asserta(type(P,A,body_pred)); true),
            (\+(user:body_pred(P/A)) -> asserta(user:body_pred(P/A)); true),
            functor(Atom,P,A),
            Atom =.. [P|Args],
            asserta((body_pred_call(P,Args):-user:Atom))
        );
...
options:-
  ...
  %  (current_predicate(max_inv_preds/1) -> true;
...
assert_prog(Prog):-
    maplist(metasub_to_clause,Prog,Prog2),
    maplist(assert_clause,Prog2),
    findall(P/A,(member(sub(_Name,P,A,_Subs),Prog)),Preds),!,
    assert_body_preds_(Preds).

assert_clause(Clause):-
    assert(user:Clause).

assert_body_preds_(Pred) :-
    forall(member(P/A,Pred),((\+(user:body_pred(P/A))->asserta(user:body_pred(P/A)));(true))).

retract_program(Prog):-
    maplist(metasub_to_clause,Prog,Prog2),
    maplist(retract_clause,Prog2),
    findall(P/A,(member(sub(_Name,P,A,_Subs),Prog)),Preds),
    maplist(retract_prim,Preds).

retract_clause(H:-_):-
    retractall((user:H)).

retract_prim(P/A):- retractall(user:body_pred(P/A)).