/***************************************** 
 * Seq_Teaching_Exp_Inverse_No_Expl Test *
 *****************************************/
...
let expName = 'seq_teaching_exp_inverse_no_expl';  // from the Builder filename that created this script
...
async function experimentInit() {
  ...
  pre_test_train_instr = new visual.TextStim({
    ...
    text: 'The tiles relate to each other in one or more of these aspects: \n\ncolour, size, position and shape. \n\nCHOOSE one of the 4 provided options that will correctly complete the grid.\n\nYou have 30 SECS for each question!',
    font: 'Open Sans',
    units: undefined, 
    pos: [(- 0.6), 0], height: 0.03,  wrapWidth: 0.4, ori: 0.0,
    color: new util.Color('white'),  opacity: undefined,
...
function PRE_TEST_TRAINRoutineEachFrame() {
  return async function () {
    ...
    if ((selected !== null && (preTestTime + selectedTime) >= preTestTotalTimeL) 
    || (selected === null && (preTestTime + t) >= preTestTotalTimeL)) {
        continueRoutine = false;
        console.log("d");
    }
    // check for quit (typically the Esc key)
...
function PRE_TEST_TRAINRoutineEnd() {
  return async function () {
    ...
    if (pre_test_train_slider.markerPos !== undefined) {
        ...
        preTestTime += t;
        psychoJS.experiment.addData("pre_test_train.response","");
    }
    return Scheduler.Event.NEXT;
  };
}
...
function PRE_TESTRoutineEachFrame() {
  return async function () {
    ...
    frameRemains = 30.6  - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
  ...
  return async function () {
    ...
    if (pre_test_slider.markerPos !== undefined) {
        ...
        psychoJS.experiment.addData("pre_test.response",
            item_image_path.split("_").slice(-1)[0].split(".")[0] === itemLabels[selected - 1]);
    } else {
        preTestTime += t;
        psychoJS.experiment.addData("pre_test.response","");
    }
...
function SORT_TRAINRoutineEnd() {
  return async function () {
    ...
    psychoJS.experiment.addData("merge_test_res.text",sort_train_res.toString());
...
function EXP_CHECKRoutineEnd() {
  return async function () {
    ...
    const mPos= exp_check_slider.getMarkerPos();
    if (mPos !== undefined || mPos !== null) { 
        psychoJS.experiment.addData("exp_check_slider.response", exp_check_slider.labels[mPos - 1]);
    } 
